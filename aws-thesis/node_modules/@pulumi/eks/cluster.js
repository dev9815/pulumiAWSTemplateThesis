...
function createCore(name, args, parent, provider) {
    ...
    dependencies_1.assertCompatibleAWSCLIExists();
    ...
    if (typeof storageClasses === "string") {
        ...
        userStorageClasses[storageClasses] = pulumi.output(storageclass_1.createStorageClass(`${name.toLowerCase()}-${storageClasses}`, storageClass, {
            parent,
        ...
        for (const key of Object.keys(storageClasses)) {
            userStorageClasses[key] = pulumi.output(storageclass_1.createStorageClass(`${name.toLowerCase()}-${key}`, storageClasses[key], {
                parent,
                provider: k8sProvider,
            }));
        }
...
function createCluster(name, self, args, opts) {
    ...
    const [nodeSecurityGroup, eksClusterIngressRule] = securitygroup_1.createNodeGroupSecurityGroup(name, {
        vpcId: core.vpcId,
    ...
    if (!skipDefaultNodeGroup) {
        defaultNodeGroup = nodegroup_1.createNodeGroup(name, Object.assign({ cluster: core }, core.nodeGroupOptions), self);
    ...
    if (args.deployDashboard) {
        ...
        dashboard_1.createDashboard(name, {}, self, provider);
    }
    return {
        core,
        clusterSecurityGroup: core.clusterSecurityGroup,
